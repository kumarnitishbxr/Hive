...
export const ProfileCanvas: React.FC = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      console.error('Failed to retrieve startup strategic profile', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateBmc = async (field: string, values: string[]) => {
    ...
    try {
      ...
      const res = await startupService.updateCanvas(updatedCanvas);
      setProfile(res.data.startup);
  ...
  const handleUpdateSwot = async (field: string, values: string[]) => {
    ...
    try {
      ...
      const res = await startupService.updateSwot(updatedSwot);
      setProfile(res.data.startup);
      setEditingField(null);
    } catch (err) {
  ...
  const handleAddCompetitor = async (e: React.FormEvent) => {
    ...
    if (!compName) return;
    try {
      const res = await startupService.addCompetitor({
        name: compName,
  ...
  const bmcStructure = [
    { key: 'keyPartners', label: 'Key Partners', desc: 'Who are your key alliances or suppliers?' },
    { key: 'keyActivities', label: 'Key Activities', desc: 'What crucial steps are required to deliver your value props?' },
    { key: 'keyResources', label: 'Key Resources', desc: 'What strategic human, financial, or tech assets are required?' },
    { key: 'valuePropositions', label: 'Value Propositions', desc: 'What core values do you solve for your customers?' },
    { key: 'customerRelationships', label: 'Customer Relationships', desc: 'What type of interactions do customer segments expect?' },
    { key: 'channels', label: 'Channels', desc: 'How do you target, sell, and support your users?' },
    { key: 'customerSegments', label: 'Customer Segments', desc: 'For whom are you creating value?' },
    { key: 'costStructure', label: 'Cost Structure', desc: 'What are the main operational cost drivers?' },
    { key: 'revenueStreams', label: 'Revenue Streams', desc: 'How does your startup generate income?' }
  ];
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'bmc' && profile && (
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="liquid-glass p-4 rounded-xl md:row-span-2 space-y-2">
          ...
          {/* Value Propositions */}
          <div className="liquid-glass p-4 rounded-xl md:row-span-2 md:col-span-1 space-y-2">
          ...
          <div className="liquid-glass p-4 rounded-xl md:col-span-2 space-y-2">
          ...
          <div className="liquid-glass p-4 rounded-xl md:col-span-3 space-y-2">
      ...
      {editingField && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          ...
          <div className="w-full max-w-md liquid-glass p-6 rounded-xl relative z-10 space-y-4">
