...
import { Plus, User, ShieldAlert, MessageCircle, MessageSquare } from 'lucide-react';

export const ValidationEngine: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'score' | 'interviews' | 'feedback' | 'surveys'>('score');
  const [ideaScore, setIdeaScore] = useState<any>(null);
  ...
  const fetchValidationData = async () => {
    try {
      const scoreRes = await validationService.getIdeaScore();
      setIdeaScore(scoreRes.data);

      const intRes = await validationService.getInterviews();
      setInterviews(intRes.data.interviews);

      const fRes = await validationService.getFeedbacks();
      setFeedbacks(fRes.data.feedbacks);

      const sRes = await validationService.getSurveys();
      setSurveys(sRes.data.surveys);
    } catch (err) {
      console.error('Failed to load validation dashboard', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLogInterview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await validationService.logInterview({
  ...
  const handleCreateSurvey = async (e: React.FormEvent) => {
    ...
    try {
      await validationService.createSurvey({
        title: surveyTitle,
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'score' && ideaScore && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 liquid-glass p-6 rounded-xl space-y-4">
      ...
      {activeTab === 'interviews' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-3 max-h-[65vh] overflow-y-auto pr-2">
            {interviews.length > 0 ? (
              interviews.map(i => (
                <div key={i._id} className="liquid-glass p-4 rounded-xl space-y-3 border-l-4" style={{ borderLeftColor: i.sentimentLabel === 'Positive' ? '#10b981' : i.sentimentLabel === 'Negative' ? '#ef4444' : '#f59e0b' }}>
          ...
          <form onSubmit={handleLogInterview} className="liquid-glass p-5 rounded-xl space-y-4 h-fit">
      ...
      {activeTab === 'feedback' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-3 max-h-[65vh] overflow-y-auto pr-2">
            {feedbacks.length > 0 ? (
              feedbacks.map(f => (
                <div key={f._id} className="liquid-glass p-4 rounded-xl space-y-2 relative">
                  <div className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-2">
                  ...
                  {f.suggestions && (
                    <div className="text-[10px] text-gray-400 bg-white/2 border border-dashed border-white/5 p-2 rounded mt-2">
                      <span className="font-bold text-indigo-300 block mb-0.5">SUGGESTED ACTION</span>
                      {f.suggestions}
                    </div>
                  )}
                </div>
          ...
          <div className="liquid-glass p-5 rounded-xl text-xs space-y-3 h-fit">
      ...
      {activeTab === 'surveys' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-3 max-h-[65vh] overflow-y-auto pr-2">
            {surveys.length > 0 ? (
              surveys.map(s => (
                <div key={s._id} className="liquid-glass p-4 rounded-xl flex items-center justify-between text-xs">
          ...
          <form onSubmit={handleCreateSurvey} className="liquid-glass p-5 rounded-xl space-y-4 h-fit">
